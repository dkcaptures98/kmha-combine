...
// U10-12 age groups get ChinHold, U13-18 get Chinups + 0.5km
...
export default function CombinePage() {
  ...
  async function handleToggleLock() {
    ...
    await fetch('/api/combine-lock', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ season: selectedSeason, locked: newLocked, combine_date: newLockDate || lock?.combine_date, notes: newLockNotes || lock?.notes }) })
    loadLocks()
  }

  async function handleSaveLockSettings() {
    await fetch('/api/combine-lock', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ season: selectedSeason, locked: lock?.locked || false, combine_date: newLockDate, notes: newLockNotes }) })
    setShowLockManager(false)
    loadLocks()
  }

  const inputBase: React.CSSProperties = { background: 'rgba(5,15,35,0.8)', border: '1px solid rgba(59,130,246,0.2)', color: 'white', borderRadius: '5px', padding: '4px 6px', fontSize: '12px', textAlign: 'center', outline: 'none', width: '100%', colorScheme: 'dark' as const }
  const lockedInput: React.CSSProperties = { ...inputBase, background: 'rgba(5,10,20,0.5)', border: '1px solid rgba(30,58,95,0.3)', color: '#334155', cursor: 'not-allowed' }

  function fmtDate(d?: string) { if (!d) return ''; return new Date(d + 'T12:00:00').toLocaleDateString('en-CA', { weekday: 'long', month: 'long', day: 'numeric', year: 'numeric' }) }
  ...
  return (
    <div style={{ paddingBottom: '48px' }}>
      <div style={{ borderBottom: '1px solid rgba(59,130,246,0.1)', padding: '24px 0 20px', marginBottom: '24px', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', flexWrap: 'wrap', gap: '12px' }}>
        <div><h1 style={{ margin: 0, fontFamily: 'var(--font-display)', fontSize: '36px', fontWeight: 700, letterSpacing: '0.06em', color: 'white' }}>ANNUAL COMBINE</h1></div>
        <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '6px', background: 'rgba(52,211,153,0.1)', border: '1px solid rgba(52,211,153,0.25)', borderRadius: '6px', padding: '6px 12px' }}><span style={{ fontSize: '12px' }}>✓</span><span style={{ fontSize: '12px', color: '#34d399', fontFamily: 'var(--font-display)', fontWeight: 600 }}>'OPEN FOR ENTRY'</span></div>
          {isAdmin && <button onClick={() => setShowLockManager(!showLockManager)} style={{ padding: '6px 14px', borderRadius: '6px', fontSize: '12px', fontFamily: 'var(--font-display)', fontWeight: 600, cursor: 'pointer', background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(59,130,246,0.2)', color: '#64748b' }}>⚙ Manage Lock</button>}
        </div>
      </div>

      {showLockManager && isAdmin && <div style={{ background: 'rgba(10,20,40,0.9)', border: '1px solid rgba(59,130,246,0.3)', borderRadius: '10px', padding: '20px', marginBottom: '20px' }}><h3 style={{ margin: '0 0 16px', fontSize: '14px', fontWeight: 600, color: '#60a5fa', fontFamily: 'var(--font-display)', letterSpacing: '0.06em', textTransform: 'uppercase' }}>Combine Lock Settings — {selectedSeason}</h3><div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: '12px', marginBottom: '16px' }}><div><label style={{ display: 'block', fontSize: '11px', color: '#475569', marginBottom: '6px', fontFamily: 'var(--font-display)', letterSpacing: '0.06em', textTransform: 'uppercase' }}>Combine Date</label><input type="date" defaultValue={lock?.combine_date || ''} onChange={e => setNewLockDate(e.target.value)} style={{ width: '100%', background: 'rgba(5,15,35,0.8)', border: '1px solid rgba(59,130,246,0.25)', color: 'white', borderRadius: '6px', padding: '8px 12px', fontSize: '13px', outline: 'none', colorScheme: 'dark' }} /></div><div><label style={{ display: 'block', fontSize: '11px', color: '#475569', marginBottom: '6px', fontFamily: 'var(--font-display)', letterSpacing: '0.06em', textTransform: 'uppercase' }}>Notes</label><input type="text" defaultValue={lock?.notes || ''} onChange={e => setNewLockNotes(e.target.value)} placeholder="e.g. Kitchener Aud, 9am start" style={{ width: '100%', background: 'rgba(5,15,35,0.8)', border: '1px solid rgba(59,130,246,0.25)', color: 'white', borderRadius: '6px', padding: '8px 12px', fontSize: '13px', outline: 'none' }} /></div></div><div style={{ display: 'flex', gap: '8px' }}><button onClick={handleSaveLockSettings} style={{ padding: '8px 20px', borderRadius: '6px', fontSize: '13px', fontFamily: 'var(--font-display)', fontWeight: 600, cursor: 'pointer', background: 'linear-gradient(135deg,#1d4ed8,#2563eb)', border: 'none', color: 'white' }}>Save Settings</button><button onClick={handleToggleLock} style={{ padding: '8px 20px', borderRadius: '6px', fontSize: '13px', fontFamily: 'var(--font-display)', fontWeight: 600, cursor: 'pointer', background: lock?.locked ? 'rgba(52,211,153,0.1)' : 'rgba(239,68,68,0.1)', border: `1px solid ${lock?.locked ? 'rgba(52,211,153,0.3)' : 'rgba(239,68,68,0.3)'}`, color: lock?.locked ? '#34d399' : '#f87171' }}>{lock?.locked ? '🔓 Unlock Combine' : '🔒 Lock Combine'}</button></div></div>}

      {lock?.combine_date && <div style={{ background: 'rgba(59,130,246,0.06)', border: '1px solid rgba(59,130,246,0.2)', borderRadius: '10px', padding: '14px 20px', marginBottom: '20px', display: 'flex', alignItems: 'center', gap: '12px' }}><span style={{ fontSize: '20px' }}>📅</span><div><p style={{ margin: '0 0 2px', fontSize: '14px', fontWeight: 700, color: 'white', fontFamily: 'var(--font-display)' }}>{selectedSeason} Annual Combine</p><p style={{ margin: 0, fontSize: '13px', color: '#60a5fa' }}>{fmtDate(lock.combine_date)}{lock.notes ? ` · ${lock.notes}` : ''}</p></div></div>}
      ...
      <div style={{ background: 'rgba(10,20,40,0.8)', border: '1px solid rgba(59,130,246,0.15)', borderRadius: '10px', padding: '16px', marginBottom: '20px' }}>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(180px, 1fr))', gap: '12px' }}>
          <div><label style={{ display: 'block', fontSize: '11px', color: '#475569', fontFamily: 'var(--font-display)', letterSpacing: '0.06em', textTransform: 'uppercase' as const, marginBottom: '6px' }}>Team</label><select value={selectedTeam} onChange={e => setSelectedTeam(e.target.value)} style={{ width: '100%', background: 'rgba(5,15,35,0.8)', border: '1px solid rgba(59,130,246,0.25)', color: 'white', borderRadius: '6px', padding: '8px 12px', fontSize: '13px', appearance: 'none' as const, outline: 'none' }}><option value="">Select team...</option>{TEAMS.map(t => <option key={t} value={t}>{t}</option>)}</select></div>
          <div><label style={{ display: 'block', fontSize: '11px', color: '#475569', fontFamily: 'var(--font-display)', letterSpacing: '0.06em', textTransform: 'uppercase' as const, marginBottom: '6px' }}>Season</label><select value={selectedSeason} onChange={e => setSelectedSeason(e.target.value)} style={{ width: '100%', background: 'rgba(5,15,35,0.8)', border: '1px solid rgba(59,130,246,0.25)', color: 'white', borderRadius: '6px', padding: '8px 12px', fontSize: '13px', appearance: 'none' as const, outline: 'none' }}>{SEASONS.map(s => <option key={s} value={s}>{s}</option>)}</select></div>
          <div><label style={{ display: 'block', fontSize: '11px', color: '#475569', fontFamily: 'var(--font-display)', letterSpacing: '0.06em', textTransform: 'uppercase' as const, marginBottom: '6px' }}>Roster Phase</label><select value={selectedRosterPhase} onChange={e => setSelectedRosterPhase(e.target.value)} style={{ width: '100%', background: 'rgba(5,15,35,0.8)', border: '1px solid rgba(59,130,246,0.25)', color: 'white', borderRadius: '6px', padding: '8px 12px', fontSize: '13px', appearance: 'none' as const, outline: 'none' }}>{ROSTER_PHASES.map(p => <option key={p} value={p}>{p}</option>)}</select></div>
          {selectedTeam && <div style={{ display: 'flex', alignItems: 'flex-end', gap: '8px' }}><div style={{ padding: '6px 12px', background: isU12Team ? 'rgba(52,211,153,0.1)' : 'rgba(59,130,246,0.1)', border: `1px solid ${isU12Team ? 'rgba(52,211,153,0.3)' : 'rgba(59,130,246,0.3)'}`, borderRadius: '6px' }}><p style={{ margin: 0, fontSize: '11px', color: isU12Team ? '#34d399' : '#60a5fa', fontFamily: 'var(--font-display)', fontWeight: 600 }}>{isU12Team ? 'U10-12: Chin Hold' : 'U13-18: Chin-Ups + 0.5km'}</p></div></div>}
        </div>
      </div>
