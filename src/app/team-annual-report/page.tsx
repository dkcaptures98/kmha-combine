...
function getTests(team:string):TestDef[]{const young=isYoungTeam(team);return[{key:'height',label:'Height',lower:false,performance:false},{key:'wingspan',label:'Wingspan',lower:false,performance:false},{key:'sprint',label:'10m Sprint',lower:true,performance:true},{key:'vertical',label:'Vertical Jump',lower:false,performance:true},young?{key:'chinhold',label:'Chin Up Hold',lower:false,performance:true}:{key:'chinups',label:'Chinups',lower:false,performance:true},{key:'broad',label:'Broad Jump',lower:false,performance:true},...(!young?[{key:'time',label:'0.5km Assault Bike Time',lower:true,performance:true},{key:'watts',label:'0.5km Assault Bike Avg Watts',lower:false,performance:true}]:[])]}
...
function sprintSeconds(v?:number|null){if(typeof v!=='number'||!Number.isFinite(v)||v<=0)return null;let n=v;while(n>10)n/=10;return n>=0.5&&n<=10?n:null}
function value(row:CombineResult|undefined,key:string){if(!row)return null;if(key==='sprint')return sprintSeconds(row.sprint);if(key==='vertical')return positiveOrNull(row.vertical);if(key==='chinhold')return countOrNull(row.chinup_hold);if(key==='chinups')return countOrNull(row.chinups);if(key==='broad')return toInches(row.broad_jump_ft,row.broad_jump_in);if(key==='height')return toInches(row.height_ft,row.height_in);if(key==='wingspan')return toInches(row.wingspan_ft,row.wingspan_in);if(key==='time')return parseTimeSeconds(row.mile02_time);if(key==='watts')return positiveOrNull(row.mile02_watts);return null}
...
function performanceChange(current:number|null,previous:number|null,lower:boolean){if(current==null||previous==null||previous===0)return null;return lower?((previous-current)/previous)*100:((current-previous)/previous)*100}
function fmtChange(v:number|null){if(v==null)return'N/A';if(Math.abs(v)<0.05)return'0.0%';return`${v>0?'+':''}${v.toFixed(1)}%`}
function changeColour(v:number|null){if(v==null||Math.abs(v)<0.05)return'#64748b';return v>0?'#059669':'#dc2626'}
...
function TeamAnnualReportContent(){
 ...
 const teamRows=rows.filter(r=>r.team===team&&r.season===season&&r.roster_phase===phase)
 ...
 const summaries=useMemo(()=>tests.map(t=>{
   ...
   return{...t,currentAvg:displayedAverage(t.key,rawCurrentAvg),change:performanceChange(matchedCurrentAvg,matchedPreviousAvg,t.lower)}
 ...
 return <div style={{fontFamily:'Arial, Helvetica, sans-serif',background:'white',color:'#0f172a',minHeight:'100vh'}}><style>{`@media print{.no-print{display:none!important}@page{margin:11mm;size:A4 portrait}.report-wrapper{max-width:100%!important;padding:16px!important}.section{break-inside:avoid}.athlete-table{font-size:9px!important}.athlete-table th,.athlete-table td{padding:7px 6px!important}}`}</style><div className="no-print" style={{position:'fixed',top:16,right:16,zIndex:100,display:'flex',gap:8}}><button onClick={()=>window.print()} style={{padding:'10px 20px',background:'#1d4ed8',color:'white',border:'none',borderRadius:8,fontWeight:700,cursor:'pointer'}}>🖨 Print / Save PDF</button><button onClick={()=>window.close()} style={{padding:'10px 16px',background:'#f1f5f9',color:'#475569',border:'none',borderRadius:8,cursor:'pointer'}}>✕ Close</button></div><div className="report-wrapper" style={{maxWidth:1040,margin:'0 auto',padding:'34px 40px 48px'}}><div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',marginBottom:22,paddingBottom:18,borderBottom:'3px solid #0f172a'}}><div><p style={{margin:'0 0 6px',fontSize:11,fontWeight:800,letterSpacing:'.12em',textTransform:'uppercase',color:'#64748b'}}>Coach Report</p><h1 style={{margin:'0 0 10px',fontSize:30,letterSpacing:'.02em'}}>TEAM ANNUAL COMBINE REPORT</h1><div style={{display:'flex',gap:10,alignItems:'center',flexWrap:'wrap'}}><span style={{background:'#0f172a',color:'white',borderRadius:5,padding:'4px 12px',fontSize:12,fontWeight:800}}>{team}</span><span style={{fontSize:12,color:'#64748b'}}>{season} · {phaseLabel(phase)}</span></div></div><div style={{textAlign:'right',fontSize:11,color:'#64748b',lineHeight:1.8}}><p style={{margin:0,fontWeight:800,color:'#0f172a'}}>KMHA Combine Tracker</p><p style={{margin:0}}>Generated: {generated}</p><p style={{margin:0}}>{teamAthletes.length} athletes listed</p></div></div><div className="section" style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:14,marginBottom:28}}><div style={{border:'1px solid #e2e8f0',borderRadius:10,padding:'14px 16px',background:'#f8fafc'}}><div style={{fontSize:9,color:'#64748b',fontWeight:800,textTransform:'uppercase'}}>Athletes Listed</div><div style={{fontSize:27,fontWeight:900,marginTop:5}}>{teamAthletes.length}</div></div><div style={{border:'1px solid #e2e8f0',borderRadius:10,padding:'14px 16px',background:'#f8fafc'}}><div style={{fontSize:9,color:'#64748b',fontWeight:800,textTransform:'uppercase'}}>Tested This Period</div><div style={{fontSize:27,fontWeight:900,marginTop:5}}>{teamRows.filter(r=>hasAnyTest(r,tests)).length}</div></div><div style={{border:'1px solid #e2e8f0',borderRadius:10,padding:'14px 16px',background:'#f8fafc'}}><div style={{fontSize:9,color:'#64748b',fontWeight:800,textTransform:'uppercase'}}>Previous Period</div><div style={{fontSize:17,fontWeight:900,marginTop:7}}>{previousTarget?.season||'N/A'}</div><div style={{fontSize:10,color:'#64748b',marginTop:2}}>{previousPeriod&&previousTarget?phaseLabel(previousTarget.phase):'No prior combine data'}</div></div></div><div className="section" style={{marginBottom:30}}><h2 style={{fontSize:13,textTransform:'uppercase',borderBottom:'2px solid #e2e8f0',paddingBottom:9}}>Athlete Results</h2><table className="athlete-table" style={{width:'100%',borderCollapse:'collapse',fontSize:10,tableLayout:'fixed'}}><thead><tr style={{background:'#0f172a'}}>{['Athlete','Status',...tests.map(t=>t.label)].map((h,i)=><th key={h} style={{...headerCell,textAlign:i===0?'left':'center'}}>{h}</th>)}</tr></thead><tbody>{teamAthletes.map((a,i)=>{const r=rowByAthlete.get(a.id);const tested=hasAnyTest(r,tests);return<tr key={a.id} style={{background:i%2?'#f8fafc':'white'}}><td style={{...bodyCell,textAlign:'left',fontWeight:800}}>{a.first_name} {a.last_name}</td><td style={{...bodyCell,color:tested?'#059669':'#dc2626',fontWeight:800}}>{tested?'Tested':'Did Not Test'}</td>{tests.map(t=><td key={t.key} style={bodyCell}>{fmt(t.key,value(r,t.key))}</td>)}</tr>})}</tbody></table></div><div className="section" style={{marginTop:30,marginBottom:24}}><h2 style={{fontSize:13,textTransform:'uppercase',borderBottom:'2px solid #e2e8f0',paddingBottom:9}}>Team Averages & Improvement</h2><table style={{width:'100%',borderCollapse:'collapse',fontSize:11}}><thead><tr style={{background:'#0f172a'}}>{['Metric','Current Avg','Change'].map((h,i)=><th key={h} style={{padding:'10px 14px',color:'white',textAlign:i===0?'left':'center'}}>{h}</th>)}</tr></thead><tbody>{summaries.map((s,i)=>{const noChange=s.key==='height'||s.key==='wingspan';return<tr key={s.key} style={{background:i%2?'#f8fafc':'white'}}><td style={{padding:'10px 14px',fontWeight:800}}>{s.label}</td><td style={{padding:'10px 14px',textAlign:'center',fontWeight:800}}>{fmt(s.key,s.currentAvg)}</td><td style={{padding:'10px 14px',textAlign:'center',fontWeight:800,color:noChange?'#94a3b8':changeColour(s.change)}}>{noChange?'—':fmtChange(s.change)}</td></tr>})}</tbody></table></div><div style={{borderTop:'1px solid #e2e8f0',paddingTop:14,marginTop:18,display:'flex',justifyContent:'space-between',fontSize:10,color:'#94a3b8'}}><span>Kitchener Minor Hockey Association · Coach Team Annual Combine Report</span><span>{generated}</span></div></div></div>
}
export default function TeamAnnualReportPage(){return <Suspense fallback={<div style={{padding:48}}>Loading...</div>}><TeamAnnualReportContent/></Suspense>}
