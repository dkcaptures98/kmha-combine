  ...
  if (athleteId && (!data || !data.length)) {
    ...
    if (rows.length) {
      ...
      const athlete = (athletesResult.data ?? []).find(a => {
        const nameKey = norm(`${a.first_name}${a.last_name}`), teamKey = norm(a.team)
...
export async function POST(request: Request) {
  ...
  const requestedTeam = String(form.get('team') ?? '').trim()
  const autoTeams = !requestedTeam || requestedTeam === '__AUTO__'
  ...
  let athleteQuery = supabase.from('athletes').select('id,first_name,last_name,team')
  ...
  for (const a of athletes ?? []) {
    ...
    roster.set(key, [...(roster.get(key) ?? []), a])
  }
  ...
  for (const row of rows.slice(1)) {
    ...
    const candidates = roster.get(`${norm(first)}|${norm(last)}`) ?? []
    if (!candidates.length) { unmatched.push(`${first} ${last}`.trim()); continue }
    if (candidates.length > 1) { ambiguous.push(`${first} ${last}`.trim()); continue }
    ...
    for (const c of dateCols) {
      ...
      sessionData[iso(c.d)] = attended; rawAttended += attended
    }
    const total = dateCols.length, bonus = Math.ceil(total * 0.07), adjusted = Math.min(total, rawAttended + bonus)
  ...
  return NextResponse.json({ team:autoTeams?'Auto-detect':requestedTeam, teams, season, first_session:iso(dateCols[0].d), last_session:iso(dateCols[dateCols.length-1].d), total_sessions:dateCols.length, bonus_sessions:Math.ceil(dateCols.length*0.07), matched, unmatched, ambiguous, saved:!preview })
}
