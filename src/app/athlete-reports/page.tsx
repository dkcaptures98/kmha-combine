...
export default function PlayerReportCardsPage() {
  ...
  useEffect(() => { fetch('/api/athletes').then(r => r.json()).then(data => { setAthletes(data); setLoading(false) }) }, [])

  function reportUrlFor(athleteId: string) { return `/athlete-report?id=${athleteId}${reportMode === 'annual' ? '&type=annual' : ''}` }
  ...
  const filtered = athletes.filter(a => { const matchTeam = !selectedTeam || a.team === selectedTeam; const matchSearch = !search || `${a.first_name} ${a.last_name}`.toLowerCase().includes(search.toLowerCase()); return matchTeam && matchSearch }).sort((a, b) => a.last_name.localeCompare(b.last_name))
  ...
  async function downloadReportZip(scope: 'team' | 'all') {
    ...
    try {
      const athletesToExport = athletes.filter(athlete => scope === 'team' ? athlete.team === selectedTeamForZip : true)
      ...
      const zipBlob = await zip.generateAsync({ type: 'blob' }); const filename = scope === 'team' ? `${safeFileName(selectedTeamForZip)}-${reportMode}-report-cards.zip` : `KMHA-all-${reportMode}-report-cards.zip`; downloadBlob(zipBlob, filename)
