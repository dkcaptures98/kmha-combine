...
export default function PlayerReportCardsPage() {
  ...
  async function downloadReportZip(scope: 'team' | 'all') {
    ...
    try {
      ...
      const zipBlob = await zip.generateAsync({ type: 'blob' }); const filename = scope === 'team' ? `${safeFileName(selectedTeamForZip)}-${reportMode}-${annualSeason}-${annualPhase}-report-cards.zip` : `KMHA-all-${reportMode}-${annualSeason}-${annualPhase}-report-cards.zip`; downloadBlob(zipBlob, filename)
