...
function TeamReportCardsPrintContent() {
  ...
  useEffect(()=>{Promise.all([fetch(`/api/athletes?team=${encodeURIComponent(team)}&season=${encodeURIComponent(annualSeason)}&roster_phase=${encodeURIComponent(phase)}&activeOnly=true`).then(r=>r.json()),fetch('/api/combine').then(r=>r.json()).catch(()=>[]),fetch('/api/entries').then(r=>r.json()).catch(()=>[])]).then(([a,c,e])=>{setAthletes(Array.isArray(a)?a:[]);setCombine(Array.isArray(c)?c:[]);setEntries(Array.isArray(e)?e:[]);setLoading(false)})},[team,annualSeason,phase])
  ...
  const annualIds = new Set(combine.filter(r=>r.season===annualSeason && r.roster_phase===phase).map(r=>r.athlete_id).filter(Boolean) as string[])
  ...
  function reportUrl(id:string){return mode==='annual'?`/athlete-annual-report?id=${id}&season=${annualSeason}&roster_phase=${phase}&packet=1`:`/athlete-report?id=${id}${regularSeason==='All Regular Testing'?'':`&season_filter=${encodeURIComponent(regularSeason)}`}`}
